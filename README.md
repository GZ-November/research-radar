...
无需安装 Python，只需要 [Docker Desktop](https://www.docker.com/products/docker-desktop/)：

```bash
git clone <仓库地址>
cd "InkMind Agent DEMO"
docker compose up
```

然后打开 `http://localhost:8501`，在左侧导航的 **设置** 页填入你的分析模型 API Key（或选择本地 Ollama），保存后立即生效。数据库和设置都保存在 `./data` 目录，容器重建不会丢失。

如果要用宿主机上的本地 Ollama：在设置页把 Ollama 地址填为 `http://host.docker.internal:11434`（Linux 还需取消 `docker-compose.yml` 里 `extra_hosts` 的注释）。
...
Demo 需要一个可用的分析模型：在 **设置** 页填远程 OpenAI-compatible API Key，或配置本地 Ollama 模型（见下文）；也可以直接编辑 `.env`（`.env.example` 有 DeepSeek 示例值）。`.env`、界面设置、SQLite 数据库和上传文稿均被 Git 忽略。
